...
import * as service from './tipos-pago.service';
import type { Actor } from '../usuarios/usuarios.service';
import type { ListTiposPagoQuery } from './tipos-pago.schema';
...
export const list = asyncHandler(async (req: Request, res: Response) => {
  const query = req.query as unknown as ListTiposPagoQuery;
...
export const setDefault = asyncHandler(async (req: Request, res: Response) => {
  res.json(await service.setDefault(Number(req.params.id), actor(req)));
});
